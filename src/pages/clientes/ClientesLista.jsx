...
const ClientesLista = () => {
  ...
  const paginado = clientes1.slice(start, end);
  const TotalPages = Math.ceil(clientes1.length / totalPerPage);
  ...
  async function handleBlur() {
    if ((await value.toLowerCase()) === "") {
      await setClientes(ClienteService.getAll());
      setClientes1(clientes);
    } else {
      await setClientes1(clientes.filter((el) => el.nome.toLowerCase() === value.toLowerCase()));
    }
  }

  useEffect(() => {
    setClientes(ClienteService.getAll());
    setClientes1(ClienteService.getAll());
  ...
  function apagar(id, nome) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire(
          "Excluido!",
          "Cliente " + nome + " excluido com sucesso!",
          "success"
        );
  ...
  return (
    <div>
      ...
      <h1>Clientes</h1>
      <br />
      <Link className="btn btn-info mb-3" to={"/clientes/create"}>
        <FaPlus /> Novo
      </Link>
      <Row>
          ...
          {paginado.map((item, i) => (
            <tr key={i}>
              ...
              <Link
                  to={
                    "/clientesDetalhe/" +
                    clientes.findIndex((cod) => cod.id === item.id)
                  }
                  title="Detalhes"
                >
